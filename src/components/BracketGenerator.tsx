...
const makeFinals = ({ games }: { games: Game[] }): Array<{ game: Game; height: number }> => {
  ...
  const gamesFeedInto = _.map(games, game => ({
    ...game,
    feedsInto: _.filter(
      games,
      ({ id, sides }) => isInGroup(id) && _.any(sides, ({ seed }) => seed && seed.sourceGame && seed.rank === 1 && seed.sourceGame.id === game.id)
    ),
  }))

  return (
    _.chain(gamesFeedInto)
