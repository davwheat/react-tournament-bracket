...
const toBracketGames = ({
  ...
  return [
    <g key={`${game.id}-${y}`}>
      <GameComponent {...rest} {...gameDimensions} key={game.id} homeOnTop={homeOnTop} game={game} x={x} y={y} />
    </g>,
  ].concat(
    _.chain(game.sides)
      ...
      .map(({ seed: { sourceGame }, side }) => {
        ...
        return [<path key={`${game.id}-${side}-${y}-path`} d={pathInfo.join(' ')} fill="transparent" stroke="black" />].concat(
          toBracketGames({
            GameComponent,
            game: sourceGame,
            homeOnTop,
