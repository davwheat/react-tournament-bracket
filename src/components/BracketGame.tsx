  ...
  render() {
      ...
      return (
        <g onMouseEnter={() => onHover(side && side.team ? side.team.id : null)} onMouseLeave={() => onHover(null)}>
          ...
          <RectClipped x={x} y={y} height={22.5} width={165}>
            <text x={x + 5} y={y + 16} style={{ ...teamNameStyle, fontStyle: side.seed && side.seed.sourcePool ? 'italic' : null }}>
              {tooltip}
    ...
    return (
      <svg width="200" height="82" viewBox="0 0 200 82" {...rest}>
        ...
        <rect x="0" y="12" width="200" height="22.5" fill={topHovered ? hoverBackgroundColor : backgroundColor} rx="3" ry="3" />
        {/* background for the bottom team */}
        <rect x="0" y="34.5" width="200" height="22.5" fill={bottomHovered ? hoverBackgroundColor : backgroundColor} rx="3" ry="3" />
        ...
        {/* the players */}
        {top ? <SideComponent x={0} y={12} side={top} onHover={onHoveredTeamIdChange} /> : null}

        {bottom ? <SideComponent x={0} y={34.5} side={bottom} onHover={onHoveredTeamIdChange} /> : null}
